...
import com.microsoft.appcenter.assets.exceptions.AssetsFinalizeException.OperationType;
...
public class FileUtils {
        ...
        for (File sourceFile : sourceFiles) {
            if (sourceFile.isDirectory()) {
                ...
                try {
                    ...
                    if (e != null) {
                        throw new AssetsFinalizeException(OperationType.COPY, e);
                    }
                }
            }
        }
    ...
    public void deleteFileOrFolderSilently(File file) {
        ...
        if (file.isDirectory()) {
            ...
            if (files == null) {
                AppCenterLog.error(Assets.LOG_TAG, "Pathname " + file.getAbsolutePath() + " doesn't denote a directory.");
                return;
            }
            for (File fileEntry : files) {
                if (fileEntry.isDirectory()) {
                    ...
                    if (!fileEntry.delete()) {
                        AppCenterLog.error(Assets.LOG_TAG, "Error deleting file " + file.getName());
                        return;
                    }
                }
            }
        ...
        for (int i = 0; i < resources.size(); i++) {
            ...
            try {
                ...
                if (logErrorMessage != null) {
                    AppCenterLog.error(Assets.LOG_TAG, logErrorMessage, e);
                }
            }
        }
